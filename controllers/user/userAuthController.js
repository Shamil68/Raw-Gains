...
const signupController = async (req, res) => {
    try {
        ...
        return res.status(statusCodes.OK).json({ success: true, message: 'User created and OTP sent successfully', redirect: `/otp?email=${encodeURIComponent(email.toLowerCase())}` });
    } catch (error) {
...
const loadSignupOtpPage = async(req,res)=>{
    ...
    if (!email) {
        return res.status(statusCodes.BAD_REQUEST).render('signup', { error: 'Email is required for OTP verification' });
    }
...
const signupOtpController = async (req, res) => {
    try {
        ...
        if (user.otp !== otp || user.expireOtp < Date.now()) {
            ...
            return res.status(statusCodes.BAD_REQUEST).json({ success: false, message: 'Invalid or OTP expired' });
        }
        ...
        return res.status(statusCodes.OK).json({ success: true, message: 'User verified successfully', redirect: '/home'});
    } catch (error) {
...
const loginController = async(req,res)=>{
    try{
        ...
        if(!user){
            return res.status(statusCodes.BAD_REQUEST).json({success:false,message:'User not exist with this email'})
        }

        if(user.isAdmin || user.role == 'admin'){
            return res.status(statusCodes.BAD_REQUEST).json({success:false,message:'admin cant login'})
        }
        ...
        if(user.isBlocked){
            return res.status(statusCodes.BAD_REQUEST).json({success:false,message:'User blocked by admin'})
        }

        const isMatch = await bcrypt.compare(password,user.password)
        if(!isMatch){
            return res.status(statusCodes.BAD_REQUEST).json({success:false,message:'User password invalid'})
        }
        ...
        return res.status(statusCodes.OK).json({success:true,message:'User login successfully',redirect:'/home' })
    }catch(error){
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({success:false,message:'Server error'})
    }
}
...
const loadForgotPasswordPage = async(req,res)=>{
    try{
        ...
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).josn({success:false,message:'Server error'})
    }
}
...
const forgotPasswordController = async(req,res)=>{
    try{
        ...
        return res.status(statusCodes.OK).json({success:true,message:'User reset otp send successfully',redirect:`/forgotPassword-otp`})
    }catch(error){
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({success:false,message:'Server error'})
    }
}
...
const forgotPasswordOtpController = async (req, res) => {
    try {
        ...
        if (!email) {
            ...
            return res.status(statusCodes.BAD_REQUEST).json({ success: false, message: 'Session expired. Please request a new OTP' });
        }
        ...
        return res.status(statusCodes.OK).json({ success: true, message: 'OTP verified successfully', redirect: '/reset-password' });
    } catch (error) {
        console.error('Verify OTP error:', error); // Detailed error log
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({ success: false, message: `Server error: ${error.message}` });
    }
};
...
const forgotPasswordResendOtpController = async(req,res)=>{
    try{
        ...
        if(!email){
            return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({success:false,message:'Session expired, please request a new OTP'})
        ...
        return res.status(statusCodes.OK).json({success:true,message:'OTP resend successfully'})

    }catch(error){
        return res.status(statusCodes.INTERNAL_SERVER_ERROR).json({success:false,message:'Serverr error'})
    }
}
...
const resetPasswordController = async (req, res) => {
    try {
        ...
        if (!email || !req.session.otpVerified) {
            ...
            return res.status(statusCodes.BAD_REQUEST).json({ success: false, message: 'Session expired or OTP not verified' });
        }
        ...
        return res.status(statusCodes.OK).json({ success: true, message: 'Password reset successfully', redirect: '/login' });
    } catch (error) {
